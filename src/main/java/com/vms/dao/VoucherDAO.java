...
public class VoucherDAO {
    ...
    private static final String SELECT_BASE =
            "SELECT v.*, d.reference as demande_ref, c.nom as client_nom, m.nom as magasin_nom " +
                    "FROM vouchers v " +
    ...
    public List<Voucher> getAllVouchers() throws SQLException {
        ...
        try (Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(SELECT_BASE + "ORDER BY v.date_emission DESC")) {
            while (rs.next()) vouchers.add(extractVoucherFromResultSet(rs));
        }
        return vouchers;
    }
    ...
    public Voucher getVoucherById(int id) throws SQLException {
        Connection conn = DatabaseConnection.getConnection();
        try (PreparedStatement pstmt = conn.prepareStatement(SELECT_BASE + "WHERE v.id = ?")) {
            pstmt.setInt(1, id);
            ResultSet rs = pstmt.executeQuery();
            if (rs.next()) return extractVoucherFromResultSet(rs);
        }
        return null;
    }
    ...
    public Voucher getVoucherByCode(String code) throws SQLException {
        Connection conn = DatabaseConnection.getConnection();
        try (PreparedStatement pstmt = conn.prepareStatement(SELECT_BASE + "WHERE v.code = ?")) {
    ...
    public boolean deleteVoucher(int id) throws SQLException {
        Connection conn = DatabaseConnection.getConnection();
        try (PreparedStatement pstmt = conn.prepareStatement("DELETE FROM vouchers WHERE id = ?")) {
            pstmt.setInt(1, id);
            return pstmt.executeUpdate() > 0;
        }
    }
    ...
    public List<Voucher> getVouchersByStatut(String statut) throws SQLException {
        ...
        Connection conn = DatabaseConnection.getConnection();
        try (PreparedStatement pstmt = conn.prepareStatement(SELECT_BASE + "WHERE v.statut = ? ORDER BY v.date_emission DESC")) {
    ...
    public List<Voucher> getVouchersByClient(int clientId) throws SQLException {
        ...
        Connection conn = DatabaseConnection.getConnection();
        try (PreparedStatement pstmt = conn.prepareStatement(SELECT_BASE + "WHERE v.client_id = ? ORDER BY v.date_emission DESC")) {
    ...
    private Voucher extractVoucherFromResultSet(ResultSet rs) throws SQLException {
        ...
        if (dateEmission != null) voucher.setDateEmission(dateEmission.toLocalDate());

        Date dateExpiration = rs.getDate("date_expiration");
        if (dateExpiration != null) voucher.setDateExpiration(dateExpiration.toLocalDate());

        Date dateUtilisation = rs.getDate("date_utilisation");
        if (dateUtilisation != null) voucher.setDateUtilisation(dateUtilisation.toLocalDate());
